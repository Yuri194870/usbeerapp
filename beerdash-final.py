...
with tab1:
    ...
    st.subheader("De data die we hier gebruiken, komt uit een gratis online bier-API met een indrukwekkende verzameling van meer dan 45.000 bieren uit verschillende landen. Om ervoor te zorgen dat onze analyses zo compleet en betrouwbaar mogelijk zijn, richten we ons specifiek op de Verenigde Staten, waar de dataset het meest volledig is. Ons dashboard is verdeeld in vier handige secties:")
    st.write("")
    st.write("")
    st.write("")
    st.subheader("Beer data per US state:")   
    ...
    st.write("")
    st.subheader("In welke staat is het bier het koudst?")
    st.write("In welke staat kun je het koudste bier vinden? Voor de echte Amerika-liefhebbers nu ook in Fahrenheit!")
    st.write("")
...
with tab2:
    ...
    keuze = st.selectbox("Selecteer de plot", ("Lokale biertjes per staat", "IBUs per staat", "Rating per staat"))
...
with tab4:
    ...
    avg_temp_per_state = avg_temp_per_state.sort_values(by=temp_col + "_num").round(1)
    ...
    fig.update_layout(yaxis=dict(categoryorder='total ascending'), xaxis=dict(range=x_range),width=800,  
        height=900  )
